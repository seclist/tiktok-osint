...
class TikTokScanner:
    ...
    def generate_activity_clock(
        self,
        slot_reserved_utc: Optional[str],
        profile_finalized_utc: Optional[str],
        video_timestamps: List[Any],
    ) -> Dict[str, Any]:
        """
        Map forensic timestamps onto a 24h UTC hour histogram (Slot Reserved, Profile Finalized,
        search-pivot video create times). Emits a compact clock face and a timezone deduction.
        """
        hours = [0] * 24

        def _add_dt_str(s: Optional[str]) -> None:
        ...
        _add_dt_str(slot_reserved_utc)
        _add_dt_str(profile_finalized_utc)

        for ct in video_timestamps:
            ts_int: Optional[int] = None
            if isinstance(ct, int):
                ts_int = ct // 1000 if ct > 10_000_000_000 else ct
            elif isinstance(ct, str) and ct.isdigit():
                ts_int = int(ct)
                ts_int = ts_int // 1000 if ts_int > 10_000_000_000 else ts_int
            if ts_int is not None and 1_000_000_000 <= ts_int <= 9_999_999_999:
        ...
        wmean = sum(h * hours[h] for h in range(24)) / total
        inferred = int(round(15.0 - wmean))
        while inferred > 14:
            inferred -= 24
        while inferred < -11:
            inferred += 24
